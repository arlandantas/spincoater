...
int tamanho = 0, velocidade = 0, tempo = 0, tempo_millis = 0;
unsigned long millis_atual = 0, millis_anterior = 0, millis_inicio = 0;
int PWM_ATUAL = 0, PWM_DESEJADO = 0;

const byte kb_cols = 3; //four rows
const byte kb_rows = 4; //three columns
...
byte kb_colPins[kb_cols] = {22,23,24}; //connect to the row pinouts of the keypad
byte kb_rowPins[kb_rows] = {26,27,28,29}; //connect to the column pinouts of the keypad
...
void loop() {
  ...
  switch (ESTADO_ATUAL) {
    ...
    case 7:
      ...
      //lcd.print("   AGUARDE...   ");
      ESTADO_ACELERACAO = 0;
      Serial.println("EXECUTANDO AGUARDE...");
      ...
      for (int i = 0; i < QTD_VELOCIDADES[tamanho]; i++) {
        ...
        if (var == -1 || n_var < var) {
          var = n_var;
          //VELOCIDADE_FINAL = VELOCIDADES[tamanho][i];
          PWM_DESEJADO = (27+i)*5;
        }
      }
      var = 0;
    ...
    case 8:
      if (millis_atual - millis_inicio >= tempo_millis || tecladoFuncaoPressionado()) {
        ...
        lcd.setCursor(0, 1);
        lcd.print("    EXECUCAO    ");
        Serial.println("FINALIZANDO EXECUCAO");
